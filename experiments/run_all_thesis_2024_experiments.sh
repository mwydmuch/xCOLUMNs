...
for e in "${EXPERIMENTS[@]}"; do
    for seed in "${SEEDS[@]}"; do
        for k in "${KS[@]}"; do
            echo "Running python3 run_thesis_2024_experiment.py $e -k $k -s $seed -v 0.0"
            python3 run_thesis_2024_experiment.py $e -k $k -s $seed -v 0.0 &
        done
    done
    wait
done
